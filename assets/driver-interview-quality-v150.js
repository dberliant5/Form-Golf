// FORM 10.73 — interview quality calibration.
...
function init(){
  ...
  // Transition is a real interview answer. Do not silently answer it for the golfer.
  if(state.transition==='unknown')state.transition='';
  state.transition=state.transition||'';

  // ----- Brand scope: leave unselected until the golfer chooses. -----
  ...
  if(!sourceBox){
    ...
    sourceBox.innerHTML=`<div class="miniTitle" style="margin-top:22px">How do you know the strike location?</div>
      ...
      <div class="options three" data-strike-source-v150>
        ...
        <button class="opt" type="button" data-v="repeated">I think that’s what I’m feeling</button>
        <button class="opt" type="button" data-v="guess">Mostly a guess</button>
      </div>`;
  ...
  // ----- Transition/tempo: deliberately no default selection. -----
  step7.innerHTML=`<div class="eyebrow">Shaft starting point</div>
    ...
    <div class="options" data-transition-v150>
      ...
      <button class="opt" type="button" data-v="unknown">Varies / not sure</button>
    </div>
  ...
  window.FORM_DRIVER_INTERVIEW_QUALITY_V153={version:'10.73',performancePriorities:PERF.map(x=>x[0])};window.FORM_DRIVER_INTERVIEW_QUALITY_V150=window.FORM_DRIVER_INTERVIEW_QUALITY_V153;return true;
}
