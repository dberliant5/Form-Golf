...
(function(){
  'use strict';
  const neutral=document.createElement('link');neutral.rel='stylesheet';neutral.href='assets/neutral-options-v91.css?v=9.2';neutral.dataset.formNeutral='true';if(!document.querySelector('link[data-form-neutral]'))document.head.appendChild(neutral);
  ...
  function resetFitStart(){if(typeof formFitStartState==='undefined')return;formFitStartState.path='';formFitStartState.categories=[];formFitStartState.depth='complete';document.querySelectorAll('#page-fitstart .fitPathCard').forEach(card=>{card.classList.remove('active','formUserSelected');});const picker=document.getElementById('fitCategoryPicker');if(picker)picker.innerHTML='';const title=document.getElementById('fitSelectionTitle');if(title)title.textContent='Choose a fitting path above.';const count=document.getElementById('fitCategoryCount');if(count)count.textContent='0';const summary=document.getElementById('fitStartSummary');if(summary)summary.textContent='Choose how you want FORM to help first.';const begin=document.getElementById('beginSelectedFits');if(begin)begin.disabled=true;}
