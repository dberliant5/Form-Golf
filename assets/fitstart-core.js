...
(function(){
  ...
  function refreshPathCopy(){
    ...
    const copy={
      ...
      complete:{label:'FULL GOLFER',title:'Fit my entire game.',bullets:['Everything in Full Bag','Golf bag','Shoes, gloves & apparel','Broader preferences that shape your FORM profile'],foot:'EQUIPMENT + EVERYTHING AROUND YOUR GAME'}
    };
    ...
    const intro=document.querySelector('#page-fitstart .fitStartIntro p');if(intro)intro.textContent='Choose one fitting, fit every club and your ball, or fit your entire game.';
  }
  ...
  function enforcePathScope(){
    ...
    if(formFitStartState.path==='equipment'){
      ...
      const title=document.getElementById('fitSelectionTitle');if(title)title.textContent='Your full equipment fit.';
  ...
  if(typeof renderFitStart==='function'){const originalRenderFitStart=renderFitStart;renderFitStart=function(){if(!formFitStartState.path){resetFitStart();return;}formFitStartState.depth='complete';originalRenderFitStart();refreshPathCopy();document.querySelector('#page-fitstart .fitDepthPanel')?.remove();enforcePathScope();const summary=document.getElementById('fitStartSummary');if(summary){const count=(formFitStartState.categories||[]).filter(id=>!formFitCategoryMeta.find(x=>x.id===id)?.future).length,labels=(formFitStartState.categories||[]).filter(id=>!formFitCategoryMeta.find(x=>x.id===id)?.future).map(id=>formFitCategoryMeta.find(x=>x.id===id)?.label).filter(Boolean);summary.textContent=count?labels.join(', '):'Choose at least one fitting to continue.';}const countEl=document.getElementById('fitCategoryCount');if(countEl)countEl.textContent=String((formFitStartState.categories||[]).length);markUserFitPath(formFitStartState.path);};}
