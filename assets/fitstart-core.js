// FORM 10.16 core — fitting taxonomy + mobile path chooser + standardized Brands navigation
(function(){
  'use strict';
  ...
  const style=document.createElement('style');style.textContent=`#page-fitstart .fitDepthPanel{display:none!important}#page-fitstart .fitStartSummaryDepth{display:none!important}#page-fitstart .fitSelectionPanel{scroll-margin-top:86px}#page-fitstart #fitCategoryPicker{scroll-margin-top:94px}#page-fitstart .fitSelectionTop{margin-top:0}#page-fitstart .fitPathCard.recommended:before,#page-fitstart .fitPathCard.recommended:after{display:none!important;content:none!important}#page-fitstart .fitPathBullets{margin:18px 0 16px;padding:0;list-style:none;display:grid;gap:8px;text-align:left}#page-fitstart .fitPathBullets li{position:relative;padding-left:18px;font-size:13px;line-height:1.45}#page-fitstart .fitPathBullets li:before{content:'•';position:absolute;left:2px;font-weight:800;color:var(--form-choice-mid)}#page-fitstart .fitPathCard.formUserSelected .fitPathBullets li:before{color:#fff}#fitScopeHelp{margin:8px 0 0;font-size:13px;line-height:1.45;color:var(--form-muted)}#driverExperience .formInlineBack{display:none!important}#step1:has(#brandQuestion:not(.hidden))~#flowNav{display:none!important}#brandQuestion .brandScopeActions{display:flex!important;justify-content:space-between!important;align-items:center!important;gap:18px!important;border-top:1px solid var(--line)!important;margin-top:20px!important;padding-top:20px!important}#brandQuestion .formBrandBottomBack{border:0!important;background:transparent!important;padding:12px 0!important;color:#737973!important;font-size:9px!important;font-weight:900!important;letter-spacing:.13em!important;text-transform:uppercase!important;min-width:0!important;width:auto!important;flex:0 0 auto!important}#brandQuestion .brandScopeConfirm{margin-left:auto!important;width:auto!important;min-width:145px!important;max-width:none!important;flex:0 0 auto!important;padding:15px 20px!important} @media(max-width:640px){#page-fitstart .fitStartShell{padding:12px 16px 24px!important;margin:0!important}#page-fitstart .fitStartIntro{display:block!important;margin:0 0 10px!important;padding:0!important}#page-fitstart .fitStartIntro .formEyebrow{display:none!important}#page-fitstart .fitStartIntro h1{font-family:Arial,Helvetica,sans-serif!important;font-size:13px!important;line-height:1.25!important;letter-spacing:.01em!important;font-weight:800!important;margin:0!important;max-width:none!important;text-transform:none!important}#page-fitstart .fitStartIntro p{display:none!important}#page-fitstart .fitPathGrid{display:grid!important;grid-template-columns:1fr!important;gap:8px!important;margin:0!important;padding:0!important}#page-fitstart .fitPathCard,#page-fitstart .fitPathCard.active,#page-fitstart .fitPathCard.recommended,#page-fitstart .fitPathCard.formUserSelected{min-height:0!important;height:auto!important;padding:13px 14px!important;margin:0!important;display:grid!important;grid-template-columns:1fr auto!important;grid-template-areas:'label foot' 'title title'!important;gap:4px 10px!important;align-items:center!important}#page-fitstart .fitPathCard .fitPathLabel{grid-area:label!important;margin:0!important;font-size:7px!important;line-height:1.2!important;letter-spacing:.14em!important}#page-fitstart .fitPathCard h2{grid-area:title!important;margin:1px 0 0!important;font-size:25px!important;line-height:1.02!important;letter-spacing:-.035em!important}#page-fitstart .fitPathCard .fitPathBullets{display:none!important}#page-fitstart .fitPathCard>span{grid-area:foot!important;margin:0!important;font-size:7px!important;line-height:1.2!important;letter-spacing:.08em!important;white-space:nowrap!important;align-self:start!important}#page-fitstart .fitSelectionPanel{margin-top:12px!important;padding-top:12px!important}#brandQuestion .brandScopeActions{margin-top:18px!important;padding-top:18px!important}#brandQuestion .formBrandBottomBack{min-height:44px!important;width:auto!important}#brandQuestion .brandScopeConfirm{min-height:44px!important;width:auto!important;min-width:145px!important;padding:15px 20px!important}}`;document.head.appendChild(style);
  ...
  function refreshPathCopy(){
    ...
    if(window.matchMedia('(max-width:640px)').matches){const h=document.querySelector('#page-fitstart .fitStartIntro h1');if(h)h.textContent='How would you like FORM to help?';}
  }

  function ensureBrandBottomBack(){
    const brand=document.getElementById('brandQuestion'),actions=brand?.querySelector('.brandScopeActions');if(!brand||!actions)return;
    brand.querySelectorAll('.formInlineBack').forEach(x=>x.remove());
    if(!actions.querySelector('.formBrandBottomBack')){
      const back=document.createElement('button');back.type='button';back.className='formBrandBottomBack';back.textContent='← Back';
      back.onclick=()=>{
        const handed=document.getElementById('handedQuestion'),step1=document.getElementById('step1'),flow=document.getElementById('flowNav');
        if(step1)step1.classList.remove('hidden');brand.classList.add('hidden');handed?.classList.remove('hidden');
        ...
        if(typeof step!=='undefined')step=1;if(flow)flow.style.display='none';
    ...
    const confirm=actions.querySelector('.brandScopeConfirm');if(confirm){confirm.style.width='auto';confirm.style.minWidth='145px';confirm.style.flex='0 0 auto';}
  }
  ...
  setTimeout(resetFitStart,0);setTimeout(ensureBrandBottomBack,0);setTimeout(ensureBrandBottomBack,500);
  document.addEventListener('click',e=>{if(e.target.closest('#driverExperience'))setTimeout(ensureBrandBottomBack,0);},true);
