// FORM 10.7 — one consistent bottom Back control throughout the driver interview.
(function(){
'use strict';
function init(){
  if(window.FORM_DRIVER_INTERVIEW_UX_V107||typeof state==='undefined')return false;
  ...
  function enforceRangeFirst(){
    const group=document.querySelector('#driverExperience [data-group="lm"]');if(!group)return;const exact=group.querySelector('.opt[data-v="exact"]');if(exact){exact.style.display='none';exact.setAttribute('aria-hidden','true');}
  ...
  ownHandedChoiceEvents();ensureHandedContinue();normalizeBrandQuestion();enforceRangeFirst();syncBottomNav();
  const obs=new MutationObserver(()=>{ownHandedChoiceEvents();ensureHandedContinue();normalizeBrandQuestion();enforceRangeFirst();removeTopBacks();});obs.observe(driver,{childList:true,subtree:true});
  ...
  window.FORM_DRIVER_INTERVIEW_UX_V104={version:'10.7'};window.FORM_DRIVER_INTERVIEW_UX_V105={version:'10.7'};window.FORM_DRIVER_INTERVIEW_UX_V106={version:'10.7'};window.FORM_DRIVER_INTERVIEW_UX_V107={version:'10.7'};return true;
}
