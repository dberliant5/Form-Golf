// FORM 8.228 — evidence-calibrated current-driver upgrade claims
(function(){
'use strict';
...
const flightLaunchMid={under8:7,'8-10':9,'10-12':11,'12-14':13,'14-16':15,'16-18':17,'18-20':19,'20plus':21};
...
function needProfile(g){
  ...
  const spin=classifyMetric('spin')||g.spin||null,launch=classifyMetric('launch')||g.traj||null,flight=flightNeeds(exactOrMid('speed')||speed);
  const curveFade=g.curveClass==='fade_curve',curveDraw=g.curveClass==='draw_curve',costlyFade=g.costly==='slice'||g.costly==='right',costlyDraw=g.costly==='hook'||g.costly==='left';
...
function applyPriorityWeights(parts,n){return parts.map(x=>{let add=0;if(['strike','direction'].includes(x.key))add+=n.accuracyW*.55;if(['speed','efficiency','carry'].includes(x.key))add+=n.distanceW*.45;if(['spin','launch'].includes(x.key))add+=n.flightW*.55;return {...x,weight:r1(x.weight+add)};});}
...
function currentProduct(g){const brand=g.currentClub?.brand,modelLabel=g.currentClub?.model||'';if(!brand||!modelLabel)return null;const clean=modelLabel.replace(/\s*\(20\d{2}\)\s*/,'').trim();const exact=products.find(p=>p.brand===brand&&(p.model===clean||p.model===modelLabel));if(exact)return {p:exact,label:modelLabel,exact:true};if(typeof currentVirtualProduct==='function'){const vp=currentVirtualProduct(g);if(vp)return {p:vp,label:modelLabel,exact:false};}return null;}
function currentScore(g){const found=currentProduct(g);if(!found)return {score:null,detail:null,label:'Insufficient product data'};let detail=scoreOne(found.p,g);if(!found.exact){const year=E.yearFromLabel(found.label);const era=E.eraEvidence(year||2018);detail={...detail,evidence:{...detail.evidence,era}};}return {score:detail.overall,detail,label:found.exact?'Exact model profile':'Historical modeled profile'};}
function compare(a,b){if(!a?.components||!b?.components)return[];const B=Object.fromEntries(b.components.map(x=>[x.key,x]));return a.components.map(x=>({key:x.key,label:x.label,delta:r1(x.score-(B[x.key]?.score??x.score))})).filter(x=>Math.abs(x.delta)>=2).sort((x,y)=>Math.abs(y.delta)-Math.abs(x.delta));}
function recommendation(best,current){if(!best)return {level:'No recommendation',text:'No eligible current-generation driver remained after your constraints.'};if(current.score==null)return {level:'Test before replacing',text:'FORM can rank new drivers for your profile, but it cannot make a defensible upgrade claim without enough product information about your current driver.'};if(current.detail?.hardConstraints?.length){const reason=current.detail.hardConstraints[0];return {level:'Worth a side-by-side test',text:`Your current driver conflicts with a core FORM fit requirement (${reason}). Because that rule prevents a normal component-by-component score, FORM recommends testing the top fit against your current driver rather than treating the fallback score as a precise upgrade gap.`};}const gap=r1(best.s.overall-current.score),diff=compare(best.s,current.detail),wins=diff.filter(x=>x.delta>=4);if(gap<2.5||wins.length===0)return {level:'No clear equipment upgrade',text:`The best new fit is ${gap.toFixed(1)} points ahead, but FORM does not see enough meaningful category improvement to call the change an upgrade.`};if(gap<6)return {level:'Worth a side-by-side test',text:`The ${gap.toFixed(1)}-point modeled advantage is driven mainly by ${wins.slice(0,2).map(x=>x.label.toLowerCase()).join(' and ')}.`};if(current.label==='Historical modeled profile')return {level:'Worth a side-by-side test',text:`FORM sees a ${gap.toFixed(1)}-point modeled fit advantage, led by ${wins.slice(0,3).map(x=>x.label.toLowerCase()).join(', ')}, but your current driver is represented by a historical modeled profile. Treat this as a strong test signal rather than a definitive replacement claim.`};const eq=current?.detail?.evidenceQuality;if(Number.isFinite(eq)&&eq<.45)return {level:'Worth a side-by-side test',text:`FORM sees a ${gap.toFixed(1)}-point modeled fit advantage, but evidence for your current driver is limited. Treat this as a strong reason to test the top fit side-by-side rather than a definitive replacement call.`};return {level:'Strong upgrade candidate',text:`FORM sees a ${gap.toFixed(1)}-point fit advantage, led by ${wins.slice(0,3).map(x=>x.label.toLowerCase()).join(', ')}.`};}
...
window.FORM_DRIVER_ENGINE_V80={scoreOne,winners,currentScore,compare};driverScoreV43=(p,g)=>scoreOne(p,g);driverRankV43=g=>winners(g);currentDriverScoreV43=g=>currentScore(g).score??75;
showResults=function(){
  ...
  try{
    ...
    grid.innerHTML=rows.slice(0,5).map((row,i)=>{const prev=i?rows[i-1]:null,rankDiff=prev?compare(row.s,prev.s):[],curDiff=cur.detail?compare(row.s,cur.detail):[];const wins=curDiff.filter(x=>x.delta>0).slice(0,3),losses=curDiff.filter(x=>x.delta<0).slice(0,2);const strongest=row.s.strengths.slice(0,2),weak=row.s.weaknesses[0];const distinction=i===0?(rows[1]?compare(row.s,rows[1].s).filter(x=>x.delta>0).slice(0,3):[]):rankDiff.filter(x=>x.delta<0).slice(0,3);return `<article class="result70Card ${i===0?'winner':''}"><div class="result70Top"><div><span class="result70Rank">${i===0?'#1 overall':'#'+(i+1)}</span><h3>${row.p.brand} ${row.p.model}</h3><small>${evidenceLabel(row.s)} · evidence ${Math.round(row.s.evidenceQuality*100)}%</small></div><div class="result70Score">${row.s.overall.toFixed(1)}<small>Fit / 100</small></div></div><div class="result70Why"><b>Why it fits you</b>${strongest.map(x=>`<p><span>${x.label}</span>${x.score.toFixed(1)}/100 — ${x.explanation}</p>`).join('')}</div><div class="result70Breakdown">${row.s.components.map(x=>`<div><span>${x.label}<small>${x.normalizedWeight.toFixed(1)}% weight</small></span><b>${x.score.toFixed(1)}</b></div>`).join('')}</div><div class="result70Distinction"><b>${i===0?'Why it leads':'Why the club above leads'}</b><p>${distinction.length?distinction.map(x=>`${x.label} ${x.delta>0?'+':''}${x.delta.toFixed(1)}`).join(' · '):(weak?`${weak.label} is the biggest limiter in this fit.`:'The evidence does not justify a larger distinction.')}</p></div><div class="result70Current"><b>Compared with your current driver</b><p>${wins.length?`Modeled advantages: ${wins.map(x=>`${x.label} +${x.delta.toFixed(1)}`).join(' · ')}`:'FORM does not identify a meaningful modeled category advantage.'}</p>${losses.length?`<p class="tradeoff">Tradeoffs: ${losses.map(x=>`${x.label} ${x.delta.toFixed(1)}`).join(' · ')}</p>`:''}</div></article>`;}).join('');
