// FORM 10.29 — scoring-boundary input integrity hardening.
(function(){
'use strict';
function init(){
  if(window.FORM_DRIVER_ENGINE_HARDENING_V129)return true;
  ...
  function integrity(){
    ...
    const launch=state.metrics?.launch?.value,spin=state.metrics?.spin?.value;
    const lowLaunch=['under8','8-10','10-12','verylow','low'].includes(launch),highLaunch=['16-18','18-20','20plus','high','veryhigh'].includes(launch);
    const lowSpin=['under1500','1500-1749','1750-1999','2000-2249','verylow','low'].includes(spin),highSpin=['3000-3499','3500plus','high','veryhigh'].includes(spin);
    if((lowLaunch&&highSpin)||(highLaunch&&lowSpin))notes.push('Launch and spin point in competing fitting directions, so configuration should be validated rather than treated as a one-variable loft fix.');
    return {status:exclude.length?'adjusted':notes.length?'review':'passed',exclude:[...new Set(exclude)],notes};
  }
  ...
  const original={scoreOne:ENG.scoreOne,winners:ENG.winners,currentScore:ENG.currentScore};
  ...
  window.FORM_DRIVER_ENGINE_HARDENING_V129={version:'10.29',original};
  return true;
}
