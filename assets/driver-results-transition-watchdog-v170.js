// FORM 10.70 — live mobile result-transition watchdog + range-first LM route cleanup.
(function(){
'use strict';
function init(){
 ...
 // With exact per-metric entry retired, the top-level LM route should not imply exact precision.
 function cleanLmRoute(){
   ...
   const range=group.querySelector('[data-v="range"]');if(range)range.textContent='Yes — I know them roughly';
   const general=group.querySelector('[data-v="general"]');if(general)general.textContent='I know the general story';
   const none=group.querySelector('[data-v="none"]');if(none)none.textContent='No — I don’t know them';
   const note=document.querySelector('#step5 .note');if(note)note.textContent='FORM uses ranges or general patterns rather than asking you to claim exact launch-monitor precision.';
 ...
 function arm(){
   ...
   watchdog=setTimeout(function(){
     ...
     try{
       ...
       console.error('FORM 10.70 results watchdog',err);
 ...
 window.FORM_DRIVER_RESULTS_WATCHDOG_V170={version:'10.70',cleanLmRoute:cleanLmRoute,arm:arm};return true;
}
