...
try{
  ...
  await page.waitForFunction(()=>window.FORM_V225_CANDIDATE_REPORT||document.getElementById('out')?.textContent?.startsWith('Error:'),null,{timeout:180000});
  const pageError=await page.evaluate(()=>document.getElementById('out')?.textContent||'');
  if(!await page.evaluate(()=>Boolean(window.FORM_V225_CANDIDATE_REPORT)))throw new Error(pageError||'Candidate report was not produced');
  ...
  const pairLite=x=>({pair:x.pair,topChanged:x.topChanged,topScoreDelta:x.topScoreDelta,carryScoreDelta:x.carryScoreDelta});
  const compact={candidate:report.candidate,passed:report.passed,checks:report.checks,summary:report.summary,thresholds:{stable:report.thresholds.stable.map(x=>({pair:`${x.a}->${x.b}`,topChanged:x.topChanged,topScoreDelta:x.topScoreDelta,moves:x.moves?.length||0})),candidate:report.thresholds.candidate.map(x=>({pair:`${x.a}->${x.b}`,topChanged:x.topChanged,topScoreDelta:x.topScoreDelta,moves:x.moves?.length||0}))},carry:{stable:report.carry?.stable?.pairs?.map(pairLite),candidate:report.carry?.candidate?.pairs?.map(pairLite)},topChanged:report.topChanged.map(x=>({id:x.id,stable:x.stableTop,candidate:x.candidateTop,delta:x.topScoreDelta,moves:x.moves})),stack:{stableM07ComponentKeys:report.stack?.stableM07ComponentKeys,candidateM07ComponentKeys:report.stack?.candidateM07ComponentKeys}};
